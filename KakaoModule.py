...
class Kakao:
  def __init__ (self, apiKey):
    ...
    self.static = {
      ...
      'ka': 'sdk/1.36.6 os/javascript lang/en-US device/Win32 origin/http%3A%2F%2Fgodzero.iptime.org:5000'
    }
  
  def AES_encrypt(self, data, key):
    connect = requests.get('https://cryptojs.holiday28784.repl.co/'+data+'^'+key)
    return connect.text
  ...
  def login (self, ID, PW):
    ...
    def authenticate ():
      #print(self.AES_encrypt(ID, self.cryptoKey))
      #print(self.AES_encrypt(PW, self.cryptoKey))
